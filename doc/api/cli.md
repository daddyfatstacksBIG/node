...
meaning that the script will be read from stdin, and the rest of the options are
...
next argument will be used as a script filename.
...
If `--cpu-prof-dir` is not specified, the generated profile will be placed in
the current working directory.

If `--cpu-prof-name` is not specified, the generated profile will be named
...
The default is `explicit`, which requires providing the full path to a module.
...
`--require` runs prior to freezing intrinsics in order to allow polyfills to be
added.
...
If `--heap-prof-dir` is not specified, the generated profile will be placed in
the current working directory.

If `--heap-prof-name` is not specified, the generated profile will be named
...
port and communicate using the [Chrome DevTools Protocol][].
...
`--preserve-symlinks-main` does not imply `--preserve-symlinks`; it is expected
that `--preserve-symlinks-main` will be used in addition to
...
If the value provided is larger than V8's maximum, then the largest value will
be chosen.
...
[customizing esm specifier resolution]:
  esm.html#esm_customizing_esm_specifier_resolution_algorithm
