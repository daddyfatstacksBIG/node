...
test("installs an npm: protocol alias package", t => {
  ...
  return mockTar({
    ...
    .then(([code, stdout, stderr]) => {
        ...
        JSON.parse(stdout),
        {
          dependencies: {
            bar: {
              ...
              resolved:
                "http://localhost:" + common.port + "/foo/-/foo-1.2.3.tgz"
            },
            foo: {
              version: "1.2.3",
              from: "foo@1.2.3",
      ...
      return common.npm(["outdated", "--json", "--registry", server.registry], {
        cwd: testDir
      });
    })
    .then(([code, stdout, stderr]) => {
      ...
      return common.npm(["update", "--registry", server.registry], {
        cwd: testDir
      });
    })
    .then(([code, stdout, stderr]) => {
    ...
    .then(([code, stdout, stderr]) => {
        ...
        parsed,
        {
          dependencies: {
            bar: {
              ...
              resolved:
                "http://localhost:" + common.port + "/foo/-/foo-1.2.4.tgz"
            },
            foo: {
              version: "1.2.4",
              from: "foo@1.2.4",
      ...
      return common.npm(["rm", "bar", "--registry", server.registry], {
        cwd: testDir
      });
    })
    .then(([code, stdout, stderr]) => {
