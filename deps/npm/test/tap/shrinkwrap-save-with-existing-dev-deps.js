...
var env = Object.keys(process.env).reduce(
  (set, key) => {
...
test("install --save leaves dev deps alone", function(t) {
  common.npm(["install", "--save", "file://" + installme], EXEC_OPTS, function(
    er,
    code,
    stdout,
    stderr
    ...
    t.ok(
      shrinkwrap.dependencies.installed,
      "save new install didn't remove dev dep"
    );
