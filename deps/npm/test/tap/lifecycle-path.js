...
test("setup", function(t) {
  fs.writeFileSync(path.join(pkg, "package.json"), JSON.stringify({}, null, 2));
  t.end();
});
...
test(
  "make sure there is no warning with a symlinked node and warn-only detection",
  {
...
function checkPath(testconfig, t) {
  ...
  if (!withDirOfCurrentNode) {
    ...
    newPATH = [path.dirname(process.execPath), PATH].join(
  ...
  common.npm(
    ...
    function(er, code, stdout, stderr) {
      ...
      var realPrependNodePathSetting =
        stdout.filter(function(line) {
      ...
      if (prependNodePathSetting === "warn-only") {
        if (symlinkedNode) {
          ...
          t.match(
            ...
            "mention the path of the binary npm itself is using."
          );
          if (extraNode) {
            ...
            t.match(
              stderr,
              regexSystemNode,
              "reports the system binary vs conflicting"
            );
          } else {
            t.match(
              stderr,
              /there is no node binary in the current PATH/,
              "informs user that there is no node binary in PATH"
            );
          }
        } else {
          t.same(stderr, "");
      ...
      var expect = expectedPaths
        .concat(newPATH.split(pathSplit))
        .map(function(p) {
          return p.replace(/\\/g, "/");
        });
