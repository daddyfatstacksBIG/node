  ...
  function _scrubFrom(deps) {
    Object.keys(deps).forEach(k => {
      ...
      if (deps[k].dependencies) {
        _scrubFrom(deps[k].dependencies);
      }
    });
  }
...
test("setup", () => {
  ...
  return rimraf(testDir)
    ...
    .then(server => {
      SERVER = server;
      return common.npm(["install", "--registry", common.registry], EXEC_OPTS);
    })
    .then(() =>
      fs.readFileAsync(path.join(testDir, "package-lock.json"), "utf8")
    )
    .then(lock => {
      RAW_LOCKFILE = lock;
    })
...
test("supports npm-shrinkwrap.json as well", t => {
  ...
  return rimraf(testDir)
    ...
    .then(() =>
      fs.readFileAsync(path.join(testDir, "npm-shrinkwrap.json"), "utf8")
    )
